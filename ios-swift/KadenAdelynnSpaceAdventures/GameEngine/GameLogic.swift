...
class GameLogic {
    ...
    func update(bounds: CGRect, currentTime: TimeInterval, timeScale: CGFloat = 1.0) {
        ...
        if newWave > gameState.wave {
            ...
            gameState.score += Int(Float(newWave * 100) * bonusMultiplier)
        }
        ...
        let baseSpawnRate = 1.5 / diffMultiplier  // Harder = faster spawning
        let waveDifficulty = Double(gameState.wave) * 0.04 * diffMultiplier
        ...
        let enemiesPerSpawn = gameState.wave > waveThreshold ? min(4, gameState.wave / waveThreshold) : 1
        
        if currentTime - lastEnemySpawn > enemySpawnRate {
            for _ in 0..<enemiesPerSpawn {
        ...
        for i in (0..<enemies.count).reversed() {
            ...
            enemy.update(bounds: bounds, timeScale: timeScale, wave: gameState.wave)
            ...
            let shootInterval = max(0.3, (baseShootInterval / diffMultiplier - waveShootBonus) / waveAggressionMultiplier)
    ...
    func shootBullet(from position: CGPoint) -> [Bullet] {
        ...
        default:
            // Single bullet for all other weapons
            let bullet = Bullet(
                position: position,
                velocity: CGPoint(x: 0, y: 10), // Move up (center-origin)
                size: CGSize(width: 5, height: 10),
                owner: .player,
                damage: 1
            )
            ...
            for j in (0..<enemies.count).reversed() {
                if enemies[j].collidesWith(bullets[i]) {
                    ...
                    if enemies[j].isDead {
                        ...
                        let baseScore = enemies[j].points
    ...
    func applyPowerUp(_ powerUp: PowerUp) {
        ...
        case .rapidFire:
            // Increase fire rate (can be handled in GameEngine)
            break
        case .shield:
            player.invulnerable = true
            player.invulnerableTimer = 5.0
        case .coin:
            gameState.coins += 1  // Each star is worth 1
