...
class ShipGraphics {
    ...
    static func createEnemyShip(size: CGSize, type: Enemy.EnemyType) -> SKNode {
        ...
        switch type {
        case .basic:
            // Basic enemies: use compact scout designs for variety
            let basicShips = ["enemy_ship_1", "enemy_ship_2", "enemy_ship_3", "enemy_ship_11"]
            imageName = basicShips.randomElement() ?? "enemy_ship_1"
        case .fast:
            // Fast enemies: use sleeker interceptor designs
            let fastShips = ["enemy_ship_4", "enemy_ship_5", "enemy_ship_12"]
            imageName = fastShips.randomElement() ?? "enemy_ship_4"
        case .tank:
            // Tank enemies: use heavier armored designs
            let tankShips = ["enemy_ship_6", "enemy_ship_7", "enemy_ship_13"]
            imageName = tankShips.randomElement() ?? "enemy_ship_6"
        case .shooter:
            // Shooter enemies: use specialized cannon designs
            let shooterShips = ["enemy_ship_8", "enemy_ship_9", "enemy_ship_10", "enemy_ship_14"]
