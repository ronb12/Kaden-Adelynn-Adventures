...
class GameStatsManager: ObservableObject {
    ...
    func updateStats(gameData: GameSessionData) {
        ...
        stats.totalPlayTime += min(max(gameData.playTime, 0), 7200)
    ...
    private func repairInvalidPlayTimeIfNeeded() {
        guard stats.totalGamesPlayed > 0 else { return }

        let maxReasonableAverageSession = 7200
        let maxReasonableTotal = stats.totalGamesPlayed * maxReasonableAverageSession
        if stats.totalPlayTime > maxReasonableTotal {
            stats.totalPlayTime = stats.totalGamesPlayed * 5 * 60
            saveStats()
        }
    }
