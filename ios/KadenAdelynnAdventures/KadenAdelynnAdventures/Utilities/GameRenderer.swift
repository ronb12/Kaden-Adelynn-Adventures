...
class GameRenderer: NSObject {
    ...
    private func renderUI(renderEncoder: MTLRenderCommandEncoder, gameState: GameState) {
        // Minimal player rendering: draw a colored rectangle for the player
        ...
        // Convert logical coordinates to Metal NDC (-1 to 1)
        let w = Float(viewportSize.width)
        let h = Float(viewportSize.height)
        let x = (px / w) * 2.0 - 1.0
        let y = -((py / h) * 2.0 - 1.0) // Metal Y is flipped
        let sx = (size / w)
        let sy = (size / h)

        // Rectangle vertices (centered at x, y)
        let vertices: [Float] = [
            x - sx, y - sy, color.x, color.y, color.z, color.w, // bottom left
            x + sx, y - sy, color.x, color.y, color.z, color.w, // bottom right
            x - sx, y + sy, color.x, color.y, color.z, color.w, // top left
            x + sx, y + sy, color.x, color.y, color.z, color.w  // top right
        ]
        ...
        let vbuf = device.makeBuffer(bytes: vertices, length: vertices.count * MemoryLayout<Float>.size, options: [])
        let ibuf = device.makeBuffer(bytes: indices, length: indices.count * MemoryLayout<UInt16>.size, options: [])
